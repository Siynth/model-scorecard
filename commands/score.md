...
---
Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/scorecard.mjs show $ARGUMENTS` and show its output verbatim. Read each bucket on its own (never across): ~0 = correctly tiered, + = beats tier, - = underperforms. `--depth N` groups models at N name segments (e.g. `--depth 1` rolls "5.6 sol"/"5.6 terra" under "5.6"); default depth comes from config. Buckets flagged ⚠ low-n are thin — treat as indicative only.
